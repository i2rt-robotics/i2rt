...
reads the three RealSense cameras locally, and records LeRobot episodes that
auto-start/stop from the teleop gate signal (the robot snapshot's
``teleop_state``).

It needs ``i2rt`` (for the portal ``RobotClient``), ``pyrealsense2`` (cameras),
``lerobot`` (dataset), and ``PyQt`` (GUI). See ``requirements.txt`` and
``README.md``.
"""
